...
export function CallbackForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (hasErrors(found)) {
      ...
      setSummary(
        missing.length === 1
          ? `One thing to check: ${found[missing[0]]}`
          : `${missing.length} things to check before we can call you back.`
      );
      ...
      if (!response.ok) {
        ...
        if (response.status === 422) {
          ...
          if (payload?.errors) {
            ...
            setStatus('idle');
            const missing = FIELD_ORDER.filter((field) => payload.errors?.[field]);
            fieldRefs.current[missing[0]]?.focus();
            return;
          }
        }
  ...
  return (
    <FormShell>
      ...
      <form onSubmit={handleSubmit} noValidate className="mt-3.5">
        ...
        {/* The submit button never greys out. If something is missing, this is
            where the visitor is told what, without losing the button. */}
        <p aria-live="polite" className="sr-only">
          {summary}
        </p>
        {summary && (
          <p className="mt-3 border border-danger/30 bg-danger/[.04] px-3 py-2 text-micro font-semibold text-danger md:text-caption">
            {summary}
          </p>
        )}
        ...
        {status === 'error' && (
          ...
          >
            <strong className="block font-bold">{copy.failure.heading}</strong>
            {copy.failure.body}
          </div>
        )}

        <button type="submit" className="btn mt-3" aria-busy={status === 'submitting'}>
...
function FormShell({ children }: { children: React.ReactNode }) {
  return (
    ...
    >
      <div className="mx-auto w-full md:max-w-2xl lg:max-w-none">{children}</div>
    </div>
  );
}
...
function ChipGroup({
  ...
  return (
    <fieldset className="mt-3.5 first:mt-0">
